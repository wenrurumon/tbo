...
setwd("/Users/wenrurumon/Documents/postdoc/gaoyuan")
...
raw <- read.csv('HA_data1234.csv')
raw <- lapply(1:4,function(i){
  rawi <- filter(raw,Period==i) %>% arrange(HID)
  rawi[,-1:-2] %>% as.matrix
})
...
phenos <- phenos[-match('LLS',names(phenos))]
...
phenos <- lapply(phenos,function(x){apply(x,2,function(x){ifelse(is.na(x),median(x,na.rm=T),x)})})
...
tmp <- pc(lapply(phenos,function(x){x[,1,drop=F]}),main='P1')
tmp <- rbind(tmp,pc(lapply(phenos,function(x){x[,2,drop=F]}),main='P2'))
tmp <- rbind(tmp,pc(lapply(phenos,function(x){x[,3,drop=F]}),main='P3'))
tmp <- rbind(tmp,pc(lapply(phenos,function(x){x[,4,drop=F]}),main='P4'))
tmp <- rbind(tmp,pc(lapply(phenos,function(x){x[,4,drop=F]-x[,1,drop=F]}),main='gap14'))
tmp <- rbind(tmp,pc(lapply(phenos,function(x){x[,4,drop=F]-x[,2,drop=F]}),main='gap24'))
tmp <- rbind(tmp,pc(lapply(phenos,function(x){x[,4,drop=F]-x[,3,drop=F]}),main='gap34'))
tmp <- rbind(tmp,pc(lapply(phenos,function(x){x[,1,drop=F]-x[,3,drop=F]}),main='gap13'))
tmp <- rbind(tmp,pc(lapply(phenos,function(x){x[,2,drop=F]-x[,3,drop=F]}),main='gap23'))
tmp <- rbind(tmp,pc(lapply(phenos,function(x){x[,2,drop=F]-x[,1,drop=F]}),main='gap12'))
tmp <- rbind(tmp,pc(phenos,main='full'))
...
simu600 <- lapply(1:1000,function(i){
  ...
  phenoi <- lapply(phenos,function(x){x[seli,]})
  pc2(phenoi)
}) 
...
tmp <- pc(phenos,main='full')
tmp <- c(tmp,pc(lapply(phenos,function(x){x[,1,drop=F]}),main='P1'))
tmp <- c(tmp,pc(lapply(phenos,function(x){x[,2,drop=F]-x[,1,drop=F]}),main='gap12'))
tmp <- c(tmp,pc(lapply(phenos,function(x){x[,2,drop=F]}),main='P2'))
tmp <- c(tmp,pc(lapply(phenos,function(x){x[,2,drop=F]-x[,3,drop=F]}),main='gap23'))
tmp <- c(tmp,pc(lapply(phenos,function(x){x[,3,drop=F]}),main='P3'))
tmp <- c(tmp,pc(lapply(phenos,function(x){x[,4,drop=F]-x[,3,drop=F]}),main='gap34'))
tmp <- c(tmp,pc(lapply(phenos,function(x){x[,4,drop=F]}),main='P4'))
...
fmap <- merge(
  fmap,
  data.table(from=names(phenos),cluster=fc2(ccap(phenos,phenos))$cluster),
  by='from'
